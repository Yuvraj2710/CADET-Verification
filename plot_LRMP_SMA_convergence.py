#!/usr/bin/env python3
"""rLRMP SMA 4comp -- FV vs DG convergence.
...
FV_LEVELS = [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096, 8192, 16384]
FV_REF = 16384

DG_POLY_DEGS = [1, 2, 3, 4, 5]
DG_NELEM = {
    1: [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096],
    2: [8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096],
...
def open_fv(z):
    path = os.path.join(BASE, f'radLRMP_FV_WENO3_SMA_4comp_FV_Z{z}.h5')
    f = h5py.File(path, 'r')
...
def open_dg(p, ne):
    path = os.path.join(BASE, f'radLRMP_DG_SMA_4comp_P{p}_DG_P{p}Z{ne}.h5')
...
def main():
    ...
    # -- CSVs --
    fv_csv = os.path.join(OUT_DIR, 'rLRMP_SMA_FV_outlet.csv')
    ...
    dg_csv = os.path.join(OUT_DIR, 'rLRMP_SMA_DG_outlet.csv')
