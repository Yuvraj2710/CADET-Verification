#!/usr/bin/env python3
"""rGRM SMA 4comp — FV vs DG convergence (Breuer thesis format).

Outlet convergence: FV Z2048 reference
No particle data available for SMA yet.

Produces:
  - rGRM_SMA_FV_outlet.csv
  - rGRM_SMA_DG_outlet.csv
...
def open_fv(z):
    ...
    return sol, sim_time, ncol, ncells, dof_bulk, dof_total
...
def open_dg(p, ne):
    ...
    return sol, sim_time, polydeg, nelem, par_polydeg, par_nelem, dof_bulk, dof_total
...
def main():
    ref_sol, _, _, _, _, _ = open_fv(FV_REF)
    print(f"Reference: FV Z{FV_REF}")

    # ── FV self-convergence ──
    fv_rows = []
    for z in FV_LEVELS:
        sol, sim_time, ncol, ncells, dof_bulk, dof_total = open_fv(z)
        row = {'NCOL': ncol, 'NCELLS': ncells,
               ...
               'Simtime': sim_time}
        if z != FV_REF:
            row['L2_outlet'], row['Linf_outlet'] = compute_errors(sol, ref_sol)
    ...
    for p in DG_POLY_DEGS:
        ...
        for ne in DG_NELEM:
            ...
            if result is None:
                ...
                continue
            sol, sim_time, polydeg, nelem, par_polydeg, par_nelem, dof_bulk, dof_total = result
            l2, linf = compute_errors(sol, ref_sol)
    ...
    make_plot(fv_dofs, fv_linf, dg_dof,
              ...
              f'rGRM SMA — Outlet convergence (ref FV Z{FV_REF})',
              'Linf_vs_DoF_outlet.png')
