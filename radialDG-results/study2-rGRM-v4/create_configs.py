#!/usr/bin/env python3
"""Create h5 input files for rGRM v4 study — very low dispersion.

Strongly reduced dispersion to show proper DG P+1 convergence rates
even for P4 over 2-3 refinement levels:
  COL_DISPERSION:  5.75e-8  -> 5.75e-12  (10000x less)
  PAR_DIFFUSION:   6.07e-10 -> 6.07e-13  (1000x less)
  FILM_DIFFUSION:  6.9e-6   -> 6.9e-9    (1000x less)

DG configs: particle and solid output enabled.
FV configs: particle output DISABLED for Z>=2048 to avoid memory blowup,
            enabled for smaller FV.
PAR_POLYDEG = POLYDEG (Breuer thesis format).

Creates:
  GRM lin 1comp:
    - DG P1-P4, nElem = 4,8,16,32,64,128
    - FV WENO3 Z16,32,64,128,256,512,1024 (with particle output)
    - FV WENO3 Z2048,4096 (outlet only — reference sims)
"""
...
OUTPUT_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          '..', '..', 'output', 'test_cadet-core', 'radialDG', 'v4')
...
# Very low dispersion parameters
LOW_DISPERSION = {
    'col_dispersion': [5.75e-12],   # 10000x less (was 5.75e-8)
    'film_diffusion': [6.9e-9],     # 1000x less (was 6.9e-6)
    'pore_diffusion': [6.07e-13],   # 1000x less (was 6.07e-10)
}
...
def apply_low_dispersion(unit_cfg):
    """Override dispersion parameters to make problem much less smooth."""
    unit_cfg['col_dispersion'] = LOW_DISPERSION['col_dispersion']
    unit_cfg['particle_type_000']['film_diffusion'] = LOW_DISPERSION['film_diffusion']
    unit_cfg['particle_type_000']['pore_diffusion'] = LOW_DISPERSION['pore_diffusion']


def set_particle_output(config_data, enabled):
    """Enable or disable particle and solid output."""
    ret = config_data['input']['return']['unit_001']
    ret['write_solution_particle'] = 1 if enabled else 0
    ret['write_solution_solid'] = 1 if enabled else 0
...
def create_DG_config(base_model, prefix, polyDeg, nElem):
    ...
    apply_low_dispersion(unit_cfg)
    set_particle_output(config_data, True)
...
def create_FV_config(base_model, prefix, nCol, particle_output=True):
    ...
    apply_low_dispersion(unit_cfg)
    set_particle_output(config_data, particle_output)
...
def main():
    ...
    print("=== GRM lin 1comp — VERY LOW DISPERSION (v4) ===")
    print(f"  COL_DISPERSION = {LOW_DISPERSION['col_dispersion'][0]:.2e}")
    print(f"  FILM_DIFFUSION = {LOW_DISPERSION['film_diffusion'][0]:.2e}")
    print(f"  PAR_DIFFUSION  = {LOW_DISPERSION['pore_diffusion'][0]:.2e}")
    print()
    ...
    for z in [16, 32, 64, 128, 256, 512, 1024]:
        name = create_FV_config(base_lin, 'radGRM_FV_WENO3_lin_1comp', z, particle_output=True)
        created.append(name)
        print(f"  {name}  (NCELLS={max(1, z // 4)}, particle=ON)")
