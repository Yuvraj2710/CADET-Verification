#!/usr/bin/env python3
"""rGRM lin 1comp — FV vs DG convergence (very low dispersion v4).

COL_DISPERSION = 5.75e-12, FILM_DIFFUSION = 6.9e-9, PAR_DIFFUSION = 6.07e-13
Reference: FV Z1024.
"""
...
BASE = os.path.join(os.path.dirname(__file__), '..', '..', '..', 'output', 'test_cadet-core', 'radialDG', 'v4-results')
...
def main():
    ...
    # Trim floor points from plots: P4 last 2, P3 last 1
    plot_trim = {1: 0, 2: 0, 3: 1, 4: 2}
    def trimmed(rows, p):
        n = plot_trim.get(p, 0)
        return rows[:len(rows) - n] if n > 0 else rows

    dg_dof = {p: (np.array([r['DoF_total'] for r in trimmed(dg_rows[p], p)]),
                  np.array([r['Linf_outlet'] for r in trimmed(dg_rows[p], p)])) for p in DG_POLY_DEGS}
