#!/usr/bin/env python3
"""rGRM lin 1comp -- FV vs DG convergence (v7: velocity_coeff = 9.775e-6).
...
FV_LEVELS = [128, 256, 512, 1024, 2048, 4096, 8192, 16384]
FV_REF = 16384
...
def main():
    ...
    def trim_plateau(rows):
        """Keep only points where error is still decreasing (positive EOC > 0.5)."""
        if len(rows) <= 1:
            return rows
        keep = [rows[0]]
        for i in range(1, len(rows)):
