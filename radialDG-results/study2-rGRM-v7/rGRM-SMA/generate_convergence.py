#!/usr/bin/env python3
"""rGRM SMA 4comp -- FV vs DG convergence (v7: velocity_coeff = 9.775e-6).
...
BASE = os.path.expanduser("~/CADET-Verification/output/test_cadet-core/radialDG/v7")

NCOMP = 4
SUM_NBOUND = 4

FV_LEVELS = [16, 32, 64, 128, 256, 512, 1024, 2048]
FV_REF = 2048
...
def open_fv(z):
    ...
    return sol_out, sim_time, ncol, ncells, dof_bulk, dof_total
...
def main():
    ref_out, _, _, _, _, _ = open_fv(FV_REF)
    print(f"Reference: FV Z{FV_REF}")

    # -- FV self-convergence --
    fv_rows = []
    for z in FV_LEVELS:
        sol_out, sim_time, ncol, ncells, dof_bulk, dof_total = open_fv(z)
    ...
    # -- CSVs --
    fv_csv = os.path.join(OUT_DIR, 'rGRM_SMA_FV_outlet.csv')
    ...
    dg_csv = os.path.join(OUT_DIR, 'rGRM_SMA_DG_outlet.csv')
    ...
    # Drop last point of P4 (hits FV reference accuracy limit)
    dg_dof = {p: (np.array([r['DoF_total'] for r in (dg_rows[p][:-1] if p == 4 else dg_rows[p])]),
                  np.array([r['Linf_outlet'] for r in (dg_rows[p][:-1] if p == 4 else dg_rows[p])])) for p in DG_POLY_DEGS}
    dg_time = {p: (np.array([r['Simtime'] for r in (dg_rows[p][:-1] if p == 4 else dg_rows[p])]),
                   np.array([r['Linf_outlet'] for r in (dg_rows[p][:-1] if p == 4 else dg_rows[p])])) for p in DG_POLY_DEGS}
