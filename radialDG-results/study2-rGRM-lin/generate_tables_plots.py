#!/usr/bin/env python3
"""Generate CSV tables and convergence plots for rGRM lin 1comp (Study 2, Config 0).

FV WENO3 self-convergence (Z128–Z8192, ref=Z8192)
DG P1–P4 convergence against FV Z8192 reference

Produces:
  - rGRM_lin_FV.csv, rGRM_lin_DG.csv
...
# GRM lin 1comp: nComp=1, nBound=[1]
NCOMP = 1
SUM_NBOUND = 1
...
def load_dg_outlet(p, z):
    path = os.path.join(BASE, f'radGRM_DG_lin_1comp_P{p}_DG_P{p}Z{z}.h5')
    f = h5py.File(path, 'r')
    sol = f['output']['solution']['unit_001']['SOLUTION_OUTLET'][:]
    sim_time = f['meta']['TIME_SIM'][()]
...
def main():
    ...
    fv_csv = os.path.join(OUT_DIR, 'rGRM_lin_FV.csv')
    ...
    for p in DG_POLY_DEGS:
        dg_rows[p] = []
        for ne in DG_NELEM:
            sol, sim_time, nelem, par_nelem, bulk_dof, total_dof = load_dg_outlet(p, ne)
            l2_err, linf_err = compute_errors(sol, ref_sol)
            dg_rows[p].append({
    ...
    dg_csv = os.path.join(OUT_DIR, 'rGRM_lin_DG.csv')
    ...
    def get_valid(rows, poly_deg):
        """Exclude points where EOC goes negative (hit FV reference floor)."""
        valid = np.ones(len(rows), dtype=bool)
        for i in range(2, len(rows)):
            if rows[i]['LinfEOC'] < -0.3:
                valid[i:] = False
                break
        # Drop extra trailing points near floor
        if poly_deg == 2:
            last_valid = np.where(valid)[0]
            if len(last_valid) > 2:
                valid[last_valid[-1]] = False
                valid[last_valid[-2]] = False
        if poly_deg == 3:
            last_valid = np.where(valid)[0]
            if len(last_valid) > 1:
                valid[last_valid[-1]] = False
